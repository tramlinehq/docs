...
:::info
Enable this in your [submission settings](/using-tramline/release-settings#submission-settings).
:::
...
:::info
Enable build queues in your [release train settings](/using-tramline/release-settings).
:::

Changes that land on the release branch can optionally be staged in a [Change Queue](/using-tramline/build-queue). The change queue gets automatically drained if either the number of changes in the queue exceed the configured limit or if the configured batch-time has passed.
...
:::info
Enable [Notifications](/using-tramline/notifications) in your [release train settings](/using-tramline/release-settings).
:::
...
:::info
Enable tagging in your [release train settings](/using-tramline/release-settings).
:::
...
By default, all builds generated by your CI workflows as artifacts are automatically uploaded to Tramline. This is true for the following file types: `aab`, `apk` and `ipa`. They are subsequently also available for download from the [Tramline dashboard](/using-tramline/release/stability) and [Slack notifications](/using-tramline/release/stability#notifications).
