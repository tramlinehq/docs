...
## Unblock upcoming releases

:::info
See [Suspending Releases](/using-tramline/release-management/suspending-releases#locking-platforms) for details.
:::

In [synchronized releases](/using-tramline/release-management/synchronized-release), locking a platform automatically unblocks upcoming releases for that platform, allowing them to proceed to production independently. This avoids one platform holding up the entire release pipeline.
